  ...
  <d2-container class="page">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="用户设置" name="userInfo">
        <el-row :gutter="20">
          <el-col :span="10" :offset="6">
            <el-form ref="userInfoForm" label-width="100px" :model="userInfo" required-asterisk :rules="userInforules"
              ...
              <el-form-item label="当前角色" prop="role">
                <el-select :value="userInfo.role" multiple placeholder="请选择" disabled size="mini" style="width: 100%;">
                  <el-option v-for="item in userInfo.role_info" :key="item.id" :label="item.name" :value="item.id">
                  </el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="拥有技能" prop="skill" v-if="showSkillInput">
                <el-input v-model="userInfo.skill" clearable></el-input>
              </el-form-item>
              <el-form-item label="目标工作" prop="job_title" v-if="showSkillInput">
                <el-select v-model="userInfo.job_title" multiple placeholder="请选择"  style="width: 100%;">
                  <el-option v-for="item in this.job_type" :key="item.id" :label="item.name" :value="item.id">
                  </el-option>
                </el-select>
              </el-form-item>
              <el-form-item>
      ...
      <el-tab-pane label="密码设置" name="passwrod">
        <el-row :gutter="20">
          <el-col :span="10" :offset="6">
            <el-form ref="userPasswordForm" :model="userPasswordInfo" required-asterisk label-width="100px"
              :label-position="position" :rules="passwordRules" center>
              ...
              <el-form-item required prop="newPassword" label="新密码">
                <el-input type="password" v-model="userPasswordInfo.newPassword" placeholder="请输入新密码"
                  clearable></el-input>
              </el-form-item>
              <el-form-item required prop="newPassword2" label="确认密码">
                <el-input type="password" v-model="userPasswordInfo.newPassword2" placeholder="请再次输入新密码"
                  clearable></el-input>
              </el-form-item>
              <el-form-item>
...
export default {
  ...
  methods: {
    ...
    getCurrentUserInfo() {
      ...
      return request({
        ...
        const job_title = res.data?.job_title?.split(',');

        _self.userInfo = { ...res.data, job_title };
        this.role_info = res.data.role_info;
      })
    },
    /**
     * 更新用户信息
    ...
    updateInfo() {
      ...
      _self.$refs.userInfoForm.validate((valid) => {
        if (valid) {
          const job_title = _self.userInfo?.job_title.map(String).join(',');
          const userInfo = { ..._self.userInfo, job_title };
  ...
  computed: {
    showSkillInput() {
      return this.role_info.some(role => role.key === "worker");
    }
  }
}
</script>
